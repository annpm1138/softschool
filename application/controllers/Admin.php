...
class Admin extends CI_Controller
{
		...
		if ($param1 == 'import_excel')
		{
			...
			foreach( $xlsx->rows() as $r ) 
			{
				...
				for( $i=0; $i < $num_cols; $i++ )
				{
                    ...
                    else if ($i == 3)   $data['religion']       =   $r[$i];
					else if ($i == 4)	$data['address']		=	$r[$i];
					else if ($i == 5)	$data['phone']			=	$r[$i];
					else if ($i == 6)	$data['email']			=	$r[$i];
					else if ($i == 7)	$data['password']		=	$r[$i];
					else if ($i == 8)	$data['roll']			=	$r[$i];
				}
    ...
    {   
        redirect(base_url() . 'index.php?admin/attendance_report/'.$this->input->post('date').'/'.
                    $this->input->post('month').'/'.
                        $this->input->post('year').'/'.
                            $this->input->post('class_id').'/'.
