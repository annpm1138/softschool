...
class Teacher extends CI_Controller
{
    ...
    function student($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'create') {
            ...
            $data['blodgroup']       = $this->input->post('blodgroup');
            $data['medicalsymptoms'] = $this->input->post('medicalsymptoms');
    ...
    function manage_attendance($date='',$month='',$year='',$class_id='',$section_id='')
    {
        ...
        if($_POST)
        {
            ...
            $this->session->set_flashdata('flash_message' , get_phrase('data_updated'));
            redirect(base_url() . 'index.php?teacher/manage_attendance/'.$date.'/'.$month.'/'.$year.'/'.$class_id.'/'.$section_id , 'refresh');
        }
        $page_data['date']     =    $date;
    ...
    function attendance_selector()
    {
     ...
     /****** ATTENDANCE REPORT *****************/
    function attendance_report($date='',$month='',$year='',$class_id='',$student_id='')
    {
        if($this->session->userdata('teacher_login')!=1)
            redirect(base_url() , 'refresh');
        ...
        $page_data['page_title'] =  get_phrase('attendance_report'). " - ".get_phrase('class')." : ".
                                            $this->crud_model->get_class_name($class_id);
        $page_data['date']        =    $date;
        $page_data['month']       =    $month;
        $page_data['year']        =    $year;
    ...
    function attendance_repselector()
    {   
        redirect(base_url() . 'index.php?teacher/attendance_report/'.$this->input->post('date').'/'.
                    $this->input->post('month').'/'.
                        $this->input->post('year').'/'.
                            $this->input->post('class_id').'/'.
